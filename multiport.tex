...
Figure~\ref{fig:mdsa-examples} gives two mDSAs adhering to this syntax. %Here is an example to illustrate the syntax of an mDSA. \textcolor{red}{(TODO)}.
 ...
 For a word $w = a_1 \dots a_{n}$, with $a_i \in \Sigma$, we write $w(i)$ to denote the letter $a_i$. The length of $w$, denoted as $|w|$, equals $n$ (the number of letters). For $j \in \{0, \dots, n-1\}$ we write $w[i, j]$ to denote the substring $a_i a_{i+1} \dots a_j$.
