...
\author{R Keerthan\inst{1,2} \and B Srivathsan\inst{2,3} \and
  ...
  \begin{abstract}
  Deterministic Suffix-reading Automata (DSA) were introduced recently as a more concise model for regular languages. %Each transition of a DSA is labeled by a word. A DSA waits in a state, listening to inputs, until one of the labels in an outgoing transition appears as a suffix of the input stream. When this happens, the corresponding transition is fired and the DSA moves to a new state. 
  DSAs can concisely capture formal specifications that are based on \emph{sequences} of input actions (and not just single actions). For a distributed system, specifications may further depend on combinations of input sequences seen across multiple components (ports). %DSAs are not equipped to reason about such concurrency in an elegant manner. 
  ...
  %EDTs have been successfully deployed in various industrial settings and several test generation tools for EDT specifications have been developed. However,  arguably, due to a complex interplay between concurrency and suffix-based event triggers, the EDT notation lacks a rigorous operational semantics. As a result, none of the test generation methods provides theoretical guarantees. We fill this gap here for a fragment of EDT: we provide formal operational semantics using mDSAs and reduce the test generation problem for the EDT fragment to emptiness of mDSAs.
  
  %that has been successfully deployed in various industrial setting. As an application, we present a formal semantics for a fragment of a specification formalism called Expressive Decision Tables (EDTs), using multi-port DSAs and reduce the test generation problem for EDTs to emptiness of multi-port DSAs. The EDT notation has been successfully deployed in an industrial setting to specify requirements and generate tests, but the notation lacks a formal operational semantics and the test generation algorithms do not give theoretical guarantees. We fill this gap here for a restricted fragment of the notation, using multi-port DSAs. 
  \end{abstract}
  % \begin{abstract}
